...
class BouncyCubeRenderer implements GLSurfaceView.Renderer 
{
    ...
    {
        if(t<=1)
            gl.glClearColor(t,0.f,0.f,1.0f);
        else if (t<=2)
            gl.glClearColor(0,t-1,0.f,1.0f);
        else if (t<=3)
            gl.glClearColor(0,0,t-2,1.0f);
        else
            t=0;
        ...
        gl.glLoadIdentity();
        gl.glTranslatef(r*(float)Math.cos(t2),0, r*(float)Math.sin(t2));

        //gl.glRotatef((float) ( (2) /2.0/Math.PI*360), 0.0f, 1.0f, 0.0f);

        gl.glRotatef((float) ( (1009.5-t2) /2.0/Math.PI*360), 0.0f, 1.0f, 0.0f);
        t+=0.01;t2+=0.001;
        if(t2>5)t2=4.5f;
